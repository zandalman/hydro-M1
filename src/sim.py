...
import thermochem as thermochem
...
def con_to_prim(u):
    ...
    w[VX]  = u[PX]/u[RHO]
    w[VY]  = u[PY]/u[RHO]
    w[P]   = (constants.gam-1)*(u[E]-u[RHO]/2*(w[VX]**2+w[VY]**2))
    w[PAS] = u[PAS]
    return w
...
def prim_to_con(w):
    ...
    u[PX]  = w[RHO]*w[VX]
    u[PY]  = w[RHO]*w[VY]
    u[E]   = w[P]/(constants.gam-1) + u[RHO]/2*(w[VX]**2+w[VY]**2)
    u[PAS] = w[PAS]
    return u
...
def hll(wface_L, wface_R, dir):
    ...
    Args
    wface_L: primative variables at left face
    wface_R: primative variables at right face
    dir: direction

    Returns
    fluxface: flux at the face
    """
    VI = [VX, VY][dir]
    ...
    csface_L   = np.sqrt(constants.gam*wface_L[P]/wface_L[RHO])
    csface_R   = np.sqrt(constants.gam*wface_R[P]/wface_R[RHO])
...
class Init(object):
    """ 
    Context manager for initial conditions.
...
class Grid(object):
    ...
    do_thermo: do thermochemistry
    c_red:    reduced speed of light
    """
    def __init__(self, N, bc_typ=NEU, bc_val=0, C=0.4, sloper=MINMOD, do_sed=True, rsolve=HLL, do_hydro=True, do_rad=True, do_thermo=True, c_red=1):
    ...
    def calc_dt(self):
        ...
        """
        if self.do_hydro:
            cs      = np.sqrt(constants.gam*self.w[P]/self.w[RHO])
    ...
    def halfstep(self):
        ...
        self.whalf[P]   = self.w[P]   - self.dt/2 * (self.w[VX]*self.dw[X,P]              + self.w[VY]*self.dw[Y,P]              + constants.gam*self.w[P]*self.dw[X,VX]   + constants.gam*self.w[P]*self.dw[Y,VY]) 
    ...
    def step_thermo(self, unit):
        """
        Full timestep for thermochemistry
        ...
        U_new = thermochem.thermochemical_step(U, dt_phys)
        ...
        self.u[RHO]      = rho_new_code
        self.u[PX]       = momx_new_code
        self.u[PY]       = momy_new_code
        self.u[E]        = energy_new_code
        self.u[ION]      = rho_HII_new_code
        self.urad[NPHOT] = N_new_code
        self.urad[XFLUX] = Fx_new_code
        self.urad[YFLUX] = Fy_new_code
    ...
    def step(self, unit, nstep=1, do_print=False):
        """ 
        Full timestep. 
        ...
        for i in range(nstep):
            ...
            if self.do_thermo: self.step_thermo(unit)
            self.t += self.dt
    ...
    def inject(self, coord, rate, xflux=0, yflux=0):
        ...
        Args
        coord: injection coordinate
        rate: injection rate
        xflux: injection x-flux
        yflux: injection y-flux
        """
